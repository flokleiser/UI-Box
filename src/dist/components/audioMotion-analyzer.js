...
exports.AudioMotionAnalyzer = void 0;
const VERSION = "4.5.0";
// internal constants
const PI = Math.PI;
const TAU = 2 * PI;
...
const validateFromList = (value, list) => value && list.includes(value) ? value : list[0];
...
class AudioMotionAnalyzer {
    constructor(container, options) {
        ...
        const config = Object.assign(Object.assign({}, DEFAULT_SETTINGS), options);
        this._ready = false;
        ...
        const canvas = (_a = options.canvas) !== null && _a !== void 0 ? _a : document.createElement("canvas");
        ...
        */
        ...
        if (config.onCanvasDraw)
            this.onCanvasDraw = config.onCanvasDraw;
        if (config.onCanvasResize)
            this.onCanvasResize = config.onCanvasResize;
    ...
    destroy() {
        if (!this._ready)
            return;
        const { canvas, _controller, _input, _merger, _observer, _ownCanvas, _splitter, } = this;
    ...
    disconnectInput(sources, stopTracks) {
        let sourceArray = [];
        if (!sources)
            sourceArray = Array.from(this._sources);
     ...
     */
    getEnergy(startFreqParam, endFreqParam) {
    ...
    registerGradient(name, options) {
        ...
        newColorStops = colorStops.map((colorStop, index) => ({
            ...
            level: typeof colorStop === "string" || isInvalid(colorStop.level)
                ? 1 - index / count
                : colorStop.level,
        }));
        ...
        newColorStops.sort((a, b) => a.level < b.level ? 1 : a.level > b.level ? -1 : 0);
    ...
    _calcBars() {
        this._bars = [];
        let bars = this._bars; // initialize object property
        if (!this._ready) {
            this._flg = {
                isAlpha: false,
                isBands: false,
        ...
        const noLedGap = _chLayout !== CHANNEL_VERTICAL || (_reflexRatio > 0 && !isLumi);
        // COMPUTE AUXILIARY VALUES
        // channelHeight is the total canvas height dedicated to each channel, including the reflex area, if any)
        const channelHeight = (canvas.height - (isDualVertical && !isLeds ? 0.5 : 0)) >>
            Number(isDualVertical);
        // analyzerHeight is the effective height used to render the analyzer, excluding the reflex area
        const analyzerHeight = (channelHeight * (isLumi || _radial ? 1 : 1 - _reflexRatio)) | 0;
        const analyzerWidth = canvas.width - centerX * Number(isDualHorizontal || _mirror !== 0);
        // channelGap is **0** if isLedDisplay == true (LEDs already have spacing); **1** if canvas height is odd (windowed); **2** if it's even
        // TODO: improve this, make it configurable?
        const channelGap = isDualVertical ? canvas.height - channelHeight * 2 : 0;
        const initialX = centerX * Number(_mirror === -1 && !isDualHorizontal && !_radial);
        let innerRadius = (Math.min(canvas.width, canvas.height) *
            0.375 *
        ...
        // }
        const barsPush = (args) => bars.push(Object.assign(Object.assign({}, args), { peak: [0, 0], hold: [0, 0], alpha: [0, 0], value: [0, 0], width: 0, barCenter: 0 }));
        /*
        ...
        */
        ...
        if (isOctaves) {
            ...
            const nearestPreferred = (value) => {
                ...
                const normalized = value / Math.pow(10, power);
                let i = 1;
            ...
            const bandWidth = _ansiBands
                ? Math.pow(10, (3 / (bands * 10)))
                : Math.pow(2, (1 / bands)); // 10^(3/10N) or 2^(1/N)
            const halfBand = Math.pow(bandWidth, 0.5);
            ...
            bars = bars.map((bar, index) => (Object.assign(Object.assign({}, bar), { posX: initialX + index * barWidth })));
            ...
            for (let i = 0, posX = 0; i < bands; i++, posX += barWidth) {
                ...
                const freq = invFreqScaling(scaleMin + (posX + barWidth / 2) / unitWidth);
            ...
            for (let i = minIndex; i <= maxIndex; i++) {
                ...
                if (posX > lastPos) {
                    ...
                    lastBar.binHi = i;
                    lastBar.freqHi = freq;
                    lastBar.freq = Math.pow((lastBar.freqLo * freq), 0.5); // compute center frequency (geometric mean)
                }
            }
        }
        /**
        ...
        if (isLeds) {
            ...
            if (customParams) {
                ...
                } while ((blockHeight < minHeight || spaceV < minHeight) &&
                    ledCount > 1);
            }
            else {
        ...
        for (const channel of [0, 1]) {
            const channelTop = _chLayout === CHANNEL_VERTICAL
                ? (channelHeight + channelGap) * channel
                : 0;
            const channelBottom = channelTop + channelHeight;
            const analyzerBottom = channelTop + analyzerHeight - (!isLeds || noLedGap ? 0 : spaceV);
    ...
    _createScales() {
        ...
        const { canvas, _frequencyScale, _mirror, _noteLabels, _radial, _scaleX, _scaleR, } = this;
        ...
        canvasR.width = canvasR.height = Math.max(minDimension * 0.15, (innerRadius << 1) + Number(isDualVertical) * scaleHeight);
        ...
        for (const item of freqLabels) {
            ...
            if (x >= prevX + labelWidthX / 2 && x <= analyzerWidth) {
                _scaleX.fillText(label.toString(), isDualHorizontal && _mirror === -1 ? analyzerWidth - x : initialX + x, y, maxW);
                if (isDualHorizontal || (_mirror && (x > labelWidthX || _mirror === 1)))
                    _scaleX.fillText(label.toString(), isDualHorizontal && _mirror !== 1
                        ? analyzerWidth + x
                        : (initialX || canvas.width) - x, y, maxW);
                prevX =
    ...
    _draw(timestamp) {
        ...
        const { analyzerHeight, centerX, centerY, channelCoords, channelHeight, channelGap, initialX, innerRadius, outerRadius, } = this._aux;
        const { _bars, canvas, _canvasGradients, _chLayout, _colorMode, _ctx, _energy, _fadePeaks, fillAlpha, _fps, _linearAmplitude, _lineWidth, maxDecibels, minDecibels, _mirror, _mode, overlay, _radial, showBgColor, showPeaks, useCanvas, _weightingFilter, } = this;
        const canvasX = this._scaleX.canvas;
        const canvasR = this._scaleR.canvas;
        const fadeFrames = (_fps * this._peakFadeTime) / 1e3;
        const fpsSquared = Math.pow(_fps, 2);
        ...
        const isTrueLeds = isLeds && this._trueLeds && _colorMode === COLOR_GRADIENT;
        ...
        const weightingdB = (freq) => {
            ...
            switch (_weightingFilter) {
                case FILTER_A: {
                    ...
                    const rA = (SQ12194 * Math.pow(f2, 2)) /
                        ((f2 + SQ20_6) *
        ...
        for (let channel = 0; channel < nChannels; channel++) {
            ...
            const bgColor = !showBgColor || (isLeds && !overlay) ? "#000" : channelGradient.bgColor;
            const radialDirection = isDualVertical && _radial && channel ? -1 : 1; // 1 = outwards, -1 = inwards
            const invertedChannel = (!channel && _mirror === -1) || (channel && _mirror === 1);
            const radialOffsetX = !isDualHorizontal || (channel && _mirror !== 1)
                ? 0
...
*/
            ...
            const drawScaleY = () => {
                ...
                const atStart = _mirror !== -1 &&
                    (!isDualHorizontal || channel === 0 || _mirror === 1);
                const atEnd = _mirror !== 1 && (!isDualHorizontal || channel !== _mirror);
                _ctx.save();
                ...
                for (let val = max; val > min; val -= incr) {
                    ...
                    if (even) {
                        ...
                        if (atEnd)
                            _ctx.fillText(val.toString(), (isDualHorizontal ? analyzerWidth : canvas.width) -
                                scaleWidth * 0.1, labelY);
                    ...
                    _ctx.beginPath();
                    _ctx.moveTo(initialX + scaleWidth * even * Number(atStart), ~~posY + 0.5); // for sharp 1px line (https://stackoverflow.com/a/13879402/2370385)
            ...
            const setBarColor = (value = 0, barIndex = 0) => {
                ...
                if ((_colorMode === COLOR_GRADIENT && !isTrueLeds) ||
                    _mode === MODE_GRAPH)
                    color = _canvasGradients[channel];
                else {
                    const selectedIndex = _colorMode === COLOR_BAR_INDEX
                        ? barIndex % colorCount
                        : colorStops.findLastIndex((item) => isLeds
            ...
            if (useCanvas) {
                ...
                if (isDualHorizontal && !_radial) {
                    const translateX = analyzerWidth * (channel + Number(invertedChannel));
                ...
                if (!overlay || showBgColor) {
                    ...
                    if (channel === 0 || (!_radial && !isDualCombined))
                        _ctx.fillRect(initialX, channelTop - channelGap, analyzerWidth, (overlay && this.reflexAlpha === 1
                            ? analyzerHeight
            ...
            for (let barIndex = 0; barIndex < nBars; barIndex++) {
                ...
                const { posX, barCenter, width, freq, binLo, binHi, ratioLo, ratioHi } = bar;
                ...
                if (bar.peak[channel] > 0 && bar.alpha[channel] > 0) {
                    ...
                    if (bar.hold[channel] < 0) {
                        if (_fadePeaks && !showPeakLine) {
                            const initialAlpha = !isAlpha || (isOutline && _lineWidth > 0)
                                ? 1
                                : isAlpha
                                    ? bar.peak[channel]
                                    : fillAlpha;
                ...
                if (peakValue > 0 &&
                    ...
                    if (isLeds) {
                        ...
                        radialPoly(posX, y, width, !this._radialInvert || isDualVertical || y + innerRadius >= 2
                            ? -2
                            : 2);
                    }
                }
    ...
    _makeGrad() {
        ...
        const { analyzerWidth, centerX, centerY, initialX, innerRadius, outerRadius, } = this._aux;
        ...
        for (const channel of [0, 1]) {
            ...
            let grad;
            if (_radial)
                grad = _ctx.createRadialGradient(centerX, centerY, outerRadius, centerX, centerY, innerRadius - (outerRadius - innerRadius) * Number(isDualVertical));
            else
                grad = _ctx.createLinearGradient(...(isHorizontal
            ...
            if (colorStops) {
                ...
                for (let channelArea = 0; channelArea < 1 + Number(dual); channelArea++) {
    ...
    _normalizedB(value) {
        ...
        const dBToLinear = (val) => Math.pow(10, (val / 20));
        ...
        if (isLinear) {
            ...
            newValue = Math.pow(dBToLinear(newValue), boost);
        }
        return clamp((newValue - minValue) / Math.pow((maxValue - minValue), boost), 0, 1);
    }
    /**
    ...
    _setCanvas(reason) {
        ...
        if (this.onCanvasResize)
            this.onCanvasResize(this._fsStatus !== undefined && this._fsStatus !== isFullscreen
                ? REASON_FSCHANGE
                : reason, this);
    }
    /**
     ...
     */
    _setGradient(name, channelParam) {
...
exports.AudioMotionAnalyzer = AudioMotionAnalyzer;
exports.default = AudioMotionAnalyzer;
