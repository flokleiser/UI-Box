...
// ???
  const buttons = ['Home', 'Settings','Buttons' , 'Spinner' , 'Particles' , 'Switches' , 'Tether' , 'Ball' , 'Joystick' , 'Test' , 'Cube' , 'Musializer' , 'Lock' , 'Lights'];
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      switch (event.key) {
        case 'h':
          setFocusedButton((prev) => (prev > 0 ? prev - 1 : buttons.length - 1));
          break;
        case 'l':
          setFocusedButton((prev) => (prev < buttons.length - 1 ? prev + 1 : 0));
          break;
        case 'Enter':
          onNavigate(buttons[focusedButton] as Page);
          break;
        default:
          break;
      }
  ...
  }, [focusedButton, buttons, onNavigate]);
      ...
      <nav>
        <div className="navbarLeft">
          {buttons.map((button, index) => (
            <button
              key={button}
              className={activePage === button ? 'navbarButton active' : 'navbarButton'}
              id={`${button.toLowerCase()}Button`}
              onClick={() => onNavigate(button as Page)}
              style={{ outline: focusedButton === index ? '2px solid blue' : 'none' }}
            >
              <span className="material-symbols-outlined">{button.toLowerCase()}</span>
            </button>
          ))}
        </div>
      </nav>
